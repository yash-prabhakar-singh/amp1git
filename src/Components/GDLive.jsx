...
import { getLivegd, getLivenc, schedulebidgdsingle, schedulebidncsingle } from './api';
import { DataGrid } from '@mui/x-data-grid';
import { Gavel, Replay } from '@mui/icons-material';
import { canBidGDLive } from './msalService';
  ...
  const columns = [
   // { field: 'platform', headerName: 'Platform', width: 110 },
    ...
    '& fieldset': { top: 0 }, color:'text.primary', fontWeight:'600',  padding: "0px 0px 0px 0px !important"}}
      ...
      <Tooltip sx={{padding:0}} title='Refreshes the list with new domains, Not refreshes data of previously detected domains'><IconButton onClick={()=>{getLivegd().then((re)=>{console.log(re.data);res.current="Refreshed!" ;setRows(re.data);setOpen(true)}
