...
import { getLivenc, getLivencUpdated, schedulebidncsingle } from '../Components/api';
import { DataGrid } from '@mui/x-data-grid';
import { Cached, Gavel, Replay } from '@mui/icons-material';
import { canBidNCLive } from '../Components/msalService';

export default function NamecheapLiveM() {
  ...
  const columns = [
   // { field: 'platform', headerName: 'Platform', width: 110 },
    ...
    },
    {
      ...
      //description: 'This column has a value getter and is not sortable.',
      //sortable: false,
      valueGetter: (params) =>{return params.row.time_left.substring(4, params.row.time_left.length)},
      type: 'date-time',
      width: 90,
    ...
    '& fieldset': { top: 0 }, color:'text.primary', fontWeight:'600',  padding: "0px 0px 0px 0px !important"}}
      ...
      <Grid  item xs={1.5}>
      <Tooltip sx={{padding:0}} title='Refreshes the list with new domains, Not refreshes data of previously detected domains'><IconButton onClick={()=>{getLivenc().then((re)=>{console.log(re.data); setRows(re.data);res.current="Refreshed";setOpen(true)}
  ).catch((err)=>{console.log(err);res.current="Not Refreshed"; setOpen1(true)});}}><Replay/></IconButton></Tooltip>
      </Grid>
      <Grid  item xs={1.5}><Tooltip sx={{color:'red',padding:0}} title='Use only when needed. Refreshes and updates each domain in Live List'><IconButton onClick={()=>{getLivencUpdated().then((res)=>{console.log(res.data); setRows(res.data);res.current="Refreshed";setOpen(true)}
