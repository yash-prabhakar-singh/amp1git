...
import { getLivenc, getLivens, schedulebidncsingle, schedulebidnssingle } from '../Components/api';
import { DataGrid } from '@mui/x-data-grid';
import { Gavel, Replay } from '@mui/icons-material';
import { canBidNSLive } from '../Components/msalService';

export default function NamesiloLiveM() {
  ...
  const columns = [
   // { field: 'platform', headerName: 'Platform', width: 110 },
    ...
    return (
      ...
      <Grid  container direction='row' xs={12} pl={1} sx={{width:'100%'}} justifyContent='flex-start' alignItems='center' columnSpacing={1} rowSpacing={3} width='100%' ><Grid item container xs={5}  justifyContent='flex-start' alignItems='flex-start'><Typography alignSelf='start'  fontWeight='bold' color='text.primary' >Namesilo Live</Typography></Grid>
        <Grid item xs={6}></Grid>
    ...
    '& fieldset': { top: 0 }, color:'text.primary', fontWeight:'600',  padding: "0px 0px 0px 0px !important"}}
      ...
      <Grid  item xs={1.5}>
      <Tooltip sx={{padding:0}} title='Refreshes the list with new domains, Not refreshes data of previously detected domains'><IconButton onClick={()=>{getLivens().then((re)=>{console.log(re.data); setRows(re.data);res.current="Refreshed";setOpen(true)}
