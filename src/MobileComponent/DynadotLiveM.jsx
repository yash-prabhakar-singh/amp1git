...
import { canBidDD, canBidDDLive } from '../Components/msalService';
import { getLive, getLiveUpdated, schedulebiddynasingle } from '../Components/api';

export default function LiveM() {
  ...
  const columns = [
   // { field: 'platform', headerName: 'Platform', width: 110 },
    ...
    return (
      ...
      <Grid  container direction='row' xs={12} pl={1} sx={{width:'100%'}} justifyContent='flex-start' alignItems='center' columnSpacing={1} rowSpacing={3} width='100%' ><Grid item container xs={5}  justifyContent='flex-start' alignItems='flex-start'><Typography alignSelf='start'  fontWeight='bold' color='text.primary' >Dynadot Live</Typography></Grid>
        <Grid item xs={6}></Grid>
    ...
    '& fieldset': { top: 0 }, color:'text.primary', fontWeight:'600',  padding: "0px 0px 0px 0px !important"}}
      ...
      <Grid  item xs={1.5}><Tooltip sx={{color:'red',padding:0}} title='Use only when needed. Refreshes and updates each domain in Live List'><IconButton onClick={()=>{getLiveUpdated().then((res)=>{console.log(res.data); setRows(res.data);res.current="Refreshed";setOpen(true)}
